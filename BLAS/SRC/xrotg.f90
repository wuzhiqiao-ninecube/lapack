...
subroutine XROTG( a, b, c, s )
   integer, parameter :: wp = kind(1.Q0)
!
...
!  ..
!  .. Intrinsic Functions ..
   intrinsic :: abs, imag, conjg, max, min, real, sqrt
!  ..
!  .. Statement Functions ..
   real(wp) :: ABSSQ
!  ..
!  .. Statement Function definitions ..
   ABSSQ( t ) = real( t,16 )**2 + imag( t )**2
!  ..
...
!
   ...
   if( g == czero ) then
      ...
      if( real(g) == zero ) then
         r = abs(imag(g))
         s = conjg( g ) / r
      elseif( imag(g) == zero ) then
         r = abs(real(g,16))
         s = conjg( g ) / r
      else
         g1 = max( abs(real(g,16)), abs(imag(g)) )
         rtmax = sqrt( safmax/2 )
